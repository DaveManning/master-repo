...
function _core_rows(FileAttachment) {
  return (
    FileAttachment("core_data@3.csv").csv({ typed: false })
  )
}

function _clean_core(core_rows) {
  return (
    // Purpose: Normalize core problem data; parse dates and coerce sales to numbers for time-series analysis.
    core_rows.map(d => ({
...
function _industry_rows(FileAttachment) {
  return (
    FileAttachment("industry_data.csv").csv({ typed: false })
  )
}

function _clean_industry(industry_rows) {
  return (
    // Purpose: Industry benchmark data; coercing revenue for scatter plot comparison.
    industry_rows.map(d => ({
      quarter: d.quarter, // Keep as string or parse if needed, looks like "2023-01-01"
      revenue: +d.revenue,
      segment: d.segment
    }))
  )
...
function _impact_rows(FileAttachment) {
  return (
    FileAttachment("QuarterlyRevenue_data@1.csv").csv({ typed: false })
  )
}

function _clean_impact(impact_rows) {
  return (
    // Purpose: Financial impact data; explicit coercion for bar chart projection.
    impact_rows.map(d => ({
      date: d.date,
      revenue_millions: +d.revenue_millions
    }))
  )
}

function _scenario_rows(FileAttachment) {
  return (
    FileAttachment("scenario_data@1.csv").csv({ typed: false })
  )
}

function _clean_scenario(scenario_rows) {
  return (
    // Purpose: Scenario modeling data; normalizing content to lower-case semantic keys.
    scenario_rows.map(d => ({
      quarter: d.Quarter,
      scenario: +d.Scenario,
      ...
      nrr: +d.NRR,
      cac_payback: +d.CAC_Payback,
      label: d.Scenario_Label
    }))
  )
}

function _pareto(FileAttachment) {
  return (
...
function _Chart1_coreProblem(Plot, core) {
  return (
    Plot.plot({
      marks: [
        Plot.ruleY([0]),
        Plot.lineY(clean_core, { x: "date", y: "sales", stroke: "category" })
      ]
    })
  )
}
...
function _Chart2_finImpact(Plot, impact) {
  return (
    Plot.plot({
      marks: [
        Plot.barY(clean_impact, {
          x: "date",
          y: "revenue_millions",
...
function _Chart3_industryBench(Plot, industry) {
  return (
    Plot.plot({
      marks: [Plot.dot(clean_industry, { x: "quarter", y: "revenue", stroke: "segment" })]
    })
  )
}
...
async function _pareto_rows(FileAttachment) {
  return (
    await FileAttachment("Pareto_data@2.csv").csv({ typed: false })
  )
}

function _clean_pareto(pareto_rows) {
  return (
    // Purpose: Pareto analysis; flattening and normalizing pain point scores for horizontal bar chart.
    pareto_rows
      .flatMap((d) => [
        {
...
function _impact_dimension(Inputs, clean_pareto) {
  return (
    Inputs.select(
...
function _pareto_filtered(clean_pareto, impact_dimension) {
  return (
    clean_pareto.filter((d) => d.dimension === impact_dimension)
  )
}

function _pareto_sorted(pareto_filtered, d3) {
  return (
    pareto_filtered
      .slice()
...
function _pareto_total(d3, pareto_sorted) {
  return (
    d3.sum(pareto_sorted, (d) => d.impact)
  )
}

function _pareto_data(pareto_sorted, d3, pareto_total) {
  return (
...
function _26(pareto_filtered) {
  return (
    pareto_filtered[0]
  )
}

function _27(pareto_total) {
  return (
    pareto_total
  )
}

function _28(pareto_data) {
  return (
    pareto_data[0]
  )
}

function _29(clean_pareto) {
  return (
    clean_pareto[0]
  )
}

function _pareto_chart(Plot, impact_dimension, pareto_data, pareto_total) {
  return (
    Plot.plot({
      marginLeft: 220,
...
function _33(Plot, scenario) {
  return (
    Plot.plot({
      marks: [
        Plot.frame(),
        Plot.ruleY([0]),
        Plot.lineY(clean_scenario, { x: "quarter", y: "gross_revenue" })
      ]
    })
  )
}

function _34(Plot, scenario) {
  return (
    Plot.plot({
      marks: [
        Plot.frame(),
        Plot.ruleY([0]),
        Plot.lineY(clean_scenario, { x: "quarter", y: "net_margins" })
      ]
    })
  )
}

function _35(Plot, scenario) {
  return (
    Plot.plot({
      marks: [
        Plot.frame(),
        Plot.ruleY([0]),
        Plot.lineY(clean_scenario, { x: "quarter", y: "nrr" })
      ]
    })
  )
}

function _36(Plot, scenario) {
  return (
    Plot.plot({
      marks: [
        Plot.frame(),
        Plot.ruleY([0]),
        Plot.lineY(clean_scenario, { x: "quarter", y: "cac_payback" })
      ]
    })
  )
}
...
export default function define(runtime, observer) {
  ...
  // Probes for debugging (House Style)
  main.variable(observer()).define(["clean_core"], (clean_core) => clean_core[0]);
  main.variable(observer()).define(["clean_industry"], (clean_industry) => clean_industry[0]);
  main.variable(observer()).define(["clean_impact"], (clean_impact) => clean_impact[0]);
  main.variable(observer()).define(["clean_scenario"], (clean_scenario) => clean_scenario[0]);

  main.variable(observer()).define(["md"], _1);
  main.variable(observer()).define(["md"], _2);
  main.variable(observer("core_rows")).define("core_rows", ["FileAttachment"], _core_rows);
  main.variable(observer("clean_core")).define("clean_core", ["core_rows"], _clean_core);
  main.variable(observer("industry_rows")).define("industry_rows", ["FileAttachment"], _industry_rows);
  main.variable(observer("clean_industry")).define("clean_industry", ["industry_rows"], _clean_industry);
  main.variable(observer("impact_rows")).define("impact_rows", ["FileAttachment"], _impact_rows);
  main.variable(observer("clean_impact")).define("clean_impact", ["impact_rows"], _clean_impact);
  main.variable(observer("scenario_rows")).define("scenario_rows", ["FileAttachment"], _scenario_rows);
  main.variable(observer("clean_scenario")).define("clean_scenario", ["scenario_rows"], _clean_scenario);
  main.variable(observer("pareto")).define("pareto", ["FileAttachment"], _pareto);
  main.variable(observer()).define(["md"], _8);
  main.variable(observer()).define(["md"], _9);
  main.variable(observer("Chart1_coreProblem")).define("Chart1_coreProblem", ["Plot", "clean_core"], _Chart1_coreProblem);
  main.variable(observer()).define(["md"], _11);
  main.variable(observer()).define(["md"], _12);
  main.variable(observer("Chart2_finImpact")).define("Chart2_finImpact", ["Plot", "clean_impact"], _Chart2_finImpact);
  main.variable(observer()).define(["md"], _14);
  main.variable(observer()).define(["md"], _15);
  main.variable(observer("Chart3_industryBench")).define("Chart3_industryBench", ["Plot", "clean_industry"], _Chart3_industryBench);
  ...
  main.variable(observer()).define(["Plot", "clean_scenario"], _33);
  main.variable(observer()).define(["Plot", "clean_scenario"], _34);
  main.variable(observer()).define(["Plot", "clean_scenario"], _35);
  main.variable(observer()).define(["Plot", "clean_scenario"], _36);
  return main;
}
